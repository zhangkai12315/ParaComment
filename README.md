...
ParaComment 是一个 Windows 桌面托盘小工具，专为“上班摸鱼看小说但不想太显眼”的场景设计。它可以把 TXT 小说或长文本自动切成一小段一小段，并伪装成 // 或 # 代码注释，直接粘贴到 VS Code、IDEA、PyCharm、Notepad++ 等编辑器里。选好小说文件后，你只需要用全局快捷键翻到下一段，编辑器里看起来像是在阅读注释、调试说明或代码备注，实际上是在低调追更。支持托盘常驻、快捷键翻页、浏览模式自动替换上一段、阅读进度保存和智能切分，让摸鱼动作更自然、更轻、更不打断工作流。
