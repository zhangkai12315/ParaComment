...
ParaComment 是一个 Windows 桌面托盘小工具，用来把 TXT 小说或长文本按“代码注释”的格式快速粘贴到当前编辑器或 IDE 中。它适合想在编辑器里低调阅读长文本、又不想频繁切窗口的人：选好文本文件后，通过全局快捷键逐段输出，内容会以 `//` 或 `#` 注释形式插入到当前光标位置。

当前项目定位是 MVP：优先保证核心链路可用、易维护、可打包成单文件 exe。
